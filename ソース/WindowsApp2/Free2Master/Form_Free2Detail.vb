...
Public Class Form_Free2Detail
  '新規:１ 、変更:２
  Public InputMode As Integer

  Public free2CodeTextValue As String
  Public free2NameTextValue As String
  ...
  Private Sub SetInitialProperty()
    Select Case InputMode
      ...
      Case 2
        ...
        CodeText.Text = free2CodeTextValue
        NameText.Text = free2NameTextValue
    End Select
  End Sub
  ...
  Function CheckValue() As Boolean
    ...
    If String.IsNullOrEmpty(CodeText.Text) Then
      errorMessages.Add("フリー2コードを入力してください。")
      CodeText.Focus()
    ElseIf CodeText.Text = "0000" Then
      errorMessages.Add("フリー2コードに0000は登録できません。")
      CodeText.Focus()
    End If
    ...
    ' 重複チェック
    If Form_Free2List.Free2Detail.Rows.Count > 0 Then
      For Each row As DataGridViewRow In Form_Free2List.Free2Detail.Rows
        If CodeText.Text.Equals(row.Cells(0).Value?.ToString()) Then
          MessageBox.Show("既に登録されているフリー2コードです。", "エラー", MessageBoxButtons.OK, MessageBoxIcon.Error)
          CodeText.Focus()
          Return False
        End If
      Next
      ...
      Try
        ...
        If confirmation = DialogResult.Yes Then
          ...
          If .Execute(sql) = 1 Then
            ...
            ' 一覧画面データ更新
            Form_Free2List.SelectFree2Master()

            Close()
          Else
            ' 登録失敗
            MessageBox.Show("フリー2マスタの登録に失敗しました。", "エラー", MessageBoxButtons.OK, MessageBoxIcon.Error)
          End If
        Else
          Exit Sub
        End If
      ...
      Try
        ...
        If confirmation = DialogResult.Yes Then
          ...
          If .Execute(sql) = 1 Then
            ...
            Dim CurrentRow As Integer = Form_Free2List.Free2Detail.CurrentRow.Index
            Form_Free2List.SelectFree2Master()
            Form_Free2List.Free2Detail.Rows(CurrentRow).Selected = True
            Form_Free2List.Free2Detail.FirstDisplayedScrollingRowIndex = CurrentRow
            '選択している行の行番号の取得
            Form_Free2List.Free2Detail.CurrentCell = Form_Free2List.Free2Detail.Rows(CurrentRow).Cells(0)
            Close()
          Else
            ' 更新失敗
            MessageBox.Show("フリー2マスタの更新に失敗しました。", "エラー", MessageBoxButtons.OK, MessageBoxIcon.Error)
          End If
        Else
          Exit Sub
        End If
  ...
  Private Function GetUpdateSql() As String
    ...
    sql &= " UPDATE MST_free2 SET"
    sql &= "     free2_name = '" & NameTxt & "',"
    sql &= "     update_date = '" & tmpdate & "'"
    sql &= " WHERE free2_number = '" & CodeTxt & "'"
  ...
  Private Sub Form_Free2Detail_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
