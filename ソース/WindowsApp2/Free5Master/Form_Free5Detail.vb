...
Public Class Form_Free5Detail
  '新規:１ 、変更:２
  Public InputMode As Integer

  Public Free5CodeTextValue As String
  Public Free5NameTextValue As String
  ...
  Private Sub SetInitialProperty()
    Select Case InputMode
      ...
      Case 2
        ...
        CodeText.Text = Free5CodeTextValue
        NameText.Text = Free5NameTextValue
    End Select
  End Sub
  ...
  Function CheckValue() As Boolean
    ...
    If String.IsNullOrEmpty(CodeText.Text) Then
      errorMessages.Add("フリー5コードを入力してください。")
      CodeText.Focus()
    ElseIf CodeText.Text = "0000" Then
      errorMessages.Add("フリー5コードに0000は登録できません。")
      CodeText.Focus()
    End If
    ...
    ' 重複チェック
    If Form_Free5List.Free5Detail.Rows.Count > 0 Then
      For Each row As DataGridViewRow In Form_Free5List.Free5Detail.Rows
        If CodeText.Text.Equals(row.Cells(0).Value?.ToString()) Then
          MessageBox.Show("既に登録されているフリー5コードです。", "エラー", MessageBoxButtons.OK, MessageBoxIcon.Error)
          CodeText.Focus()
          Return False
        End If
      Next
      ...
      Try
        ...
        If confirmation = DialogResult.Yes Then
          ...
          If .Execute(sql) = 1 Then
            ...
            ' 一覧画面データ更新
            Form_Free5List.SelectFree5Master()

            Close()
          Else
            ' 登録失敗
            MessageBox.Show("フリー5マスタの登録に失敗しました。", "エラー", MessageBoxButtons.OK, MessageBoxIcon.Error)
          End If
        Else
          Exit Sub
        End If
      ...
      Try
        ...
        If confirmation = DialogResult.Yes Then
          ...
          If .Execute(sql) = 1 Then
            ...
            Dim CurrentRow As Integer = Form_Free5List.Free5Detail.CurrentRow.Index
            Form_Free5List.SelectFree5Master()
            Form_Free5List.Free5Detail.Rows(CurrentRow).Selected = True
            Form_Free5List.Free5Detail.FirstDisplayedScrollingRowIndex = CurrentRow
            '選択している行の行番号の取得
            Form_Free5List.Free5Detail.CurrentCell = Form_Free5List.Free5Detail.Rows(CurrentRow).Cells(0)
            Close()
          Else
            ' 更新失敗
            MessageBox.Show("フリー5マスタの更新に失敗しました。", "エラー", MessageBoxButtons.OK, MessageBoxIcon.Error)
          End If
        Else
          Exit Sub
        End If
  ...
  Private Function GetUpdateSql() As String
    ...
    sql &= " UPDATE MST_Free5 SET"
    sql &= "     Free5_name = '" & NameTxt & "',"
    sql &= "     update_date = '" & tmpdate & "'"
    sql &= " WHERE free5_number = '" & CodeTxt & "'"
  ...
  Private Sub Form_Free5Detail_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
