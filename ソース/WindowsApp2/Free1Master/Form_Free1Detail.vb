...
Public Class Form_Free1Detail
  '新規:１ 、変更:２
  Public InputMode As Integer

  Public free1CodeTextValue As String
  Public free1NameTextValue As String
  ...
  Private ReadOnly Property SqlServer As ClsSqlServer
    ...
    End Get
  End Property
  Private Sub Form_Free1Detail_Load(sender As Object, e As EventArgs) Handles MyBase.Load
  ...
  Private Sub SetInitialProperty()
    Select Case InputMode
      ...
      Case 2
        ...
        CodeText.Text = free1CodeTextValue
        NameText.Text = free1NameTextValue
    End Select
  End Sub
  ...
  Function CheckValue() As Boolean
    ...
    If String.IsNullOrEmpty(CodeText.Text) Then
      errorMessages.Add("フリー1コードを入力してください。")
      CodeText.Focus()
    ElseIf CodeText.Text = "0000" Then
      errorMessages.Add("フリー1コードに0000は登録できません。")
      CodeText.Focus()
    End If
    ...
    ' 重複チェック
    If Form_Free1List.Free1Detail.Rows.Count > 0 Then
      For Each row As DataGridViewRow In Form_Free1List.Free1Detail.Rows
        If CodeText.Text.Equals(row.Cells(0).Value?.ToString()) Then
          MessageBox.Show("既に登録されているフリー1コードです。", "エラー", MessageBoxButtons.OK, MessageBoxIcon.Error)
          CodeText.Focus()
          Return False
        End If
      Next
      ...
      Try
        ...
        If confirmation = DialogResult.Yes Then
          ...
          If .Execute(sql) = 1 Then
            ...
            ' 一覧画面データ更新
            Form_Free1List.SelectFree1Master()

            Close()
          Else
            ' 登録失敗
            MessageBox.Show("フリー1マスタの登録に失敗しました。", "エラー", MessageBoxButtons.OK, MessageBoxIcon.Error)
          End If
        Else
          Exit Sub
        End If
      ...
      Try
        ...
        If confirmation = DialogResult.Yes Then
          ...
          If .Execute(sql) = 1 Then
            ...
            Dim CurrentRow As Integer = Form_Free1List.Free1Detail.CurrentRow.Index
            Form_Free1List.SelectFree1Master()
            Form_Free1List.Free1Detail.Rows(CurrentRow).Selected = True
            Form_Free1List.Free1Detail.FirstDisplayedScrollingRowIndex = CurrentRow
            '選択している行の行番号の取得
            Form_Free1List.Free1Detail.CurrentCell = Form_Free1List.Free1Detail.Rows(CurrentRow).Cells(0)
            Close()
          Else
            ' 更新失敗
            MessageBox.Show("フリー1マスタの更新に失敗しました。", "エラー", MessageBoxButtons.OK, MessageBoxIcon.Error)
          End If
        Else
          Exit Sub
        End If
  ...
  Private Function GetUpdateSql() As String
    ...
    sql &= " UPDATE MST_Free1 SET"
    sql &= "     free1_name = '" & NameTxt & "',"
    sql &= "     update_date = '" & tmpdate & "'"
    sql &= " WHERE free1_number = '" & CodeTxt & "'"
  ...
  Private Sub Form_Free1Detail_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
