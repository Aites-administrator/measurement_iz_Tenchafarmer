...
Public Class Form_StaffList
  ...
  '    Dim totalRows As Integer = 140
  '    Dim blockRows As Integer = 70
  '    Dim outputMaxRows As Integer = 74 ' 3（ヘッダー） + 70データ + 余白

  '    Dim wb As New XLWorkbook()
  '    Dim ws = wb.Worksheets.Add("担当者一覧")
  ...
  '     === ヘッダー行（3行目） ===
  '    ws.Cell(3, 1).Value = "担当者コード"
  '    ws.Cell(3, 2).Value = "担当者名"

  '    ws.Cell(3, 6).Value = "担当者コード"
  '    ws.Cell(3, 7).Value = "担当者名"

  '    バーコードヘッダー結合
  '    ws.Range(3, 3, 3, 4).Merge()
  '    ws.Range(3, 8, 3, 9).Merge()

  '    ws.Cell(3, 3).Value = "バーコード"
  '    ws.Cell(3, 8).Value = "バーコード"

  '    ヘッダー中央寄せ
  '    ws.Range(3, 1, 3, 9).Style.Font.Bold = True
  '    ws.Range(3, 1, 3, 9).Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Center
  '    ws.Range(3, 1, 3, 9).Style.Alignment.Vertical = XLAlignmentVerticalValues.Center
  '    ws.Row(3).Height = 20

  '     === データ出力 ===
  '    For i As Integer = 0 To totalRows - 1
  '      Dim rowIndexL As Integer = i + 4 ' データ開始行は4
  '      If i < OutputDt.Rows.Count Then
  '        Dim code = OutputDt.Rows(i)("担当者コード").ToString()
  '        Dim name = OutputDt.Rows(i)("担当者名").ToString()
  '        Dim barcode = "*" & code & "*"

  '        If i < blockRows Then
  '          左ブロック（A〜E）
  '          ws.Cell(rowIndexL, 1).Value = code
  '          ws.Cell(rowIndexL, 2).Value = name

  '          Dim barcodeCol = If(i Mod 2 = 0, 3, 4)
  '          ws.Cell(rowIndexL, barcodeCol).Value = barcode
  '          With ws.Cell(rowIndexL, barcodeCol).Style
  ...
  '          Dim rowIndexR As Integer = rowIndexL - 70

  '          ws.Cell(rowIndexR, 6).Value = code
  '          ws.Cell(rowIndexR, 7).Value = name

  '          Dim barcodeCol = If(j Mod 2 = 0, 8, 9)
  '          ws.Cell(rowIndexR, barcodeCol).Value = barcode
  '          With ws.Cell(rowIndexR, barcodeCol).Style
  '            .Font.FontName = "Code39"
  '            .Font.FontSize = 20
  '            .Alignment.Horizontal = XLAlignmentHorizontalValues.Center
  '            .Alignment.Vertical = XLAlignmentVerticalValues.Center
  ...
  '     === 罫線・整形 ===
  '    For row = 3 To 3 + blockRows ' 74行まで
  '      ws.Row(row).Height = 20

  '      For col = 1 To 9 ' J列はスキップ
  '        With ws.Cell(row, col).Style
  ...
  '    保存
  '    wb.SaveAs(filePath)
  '    MessageBox.Show("Excel出力が完了しました。" & vbCrLf & filePath, "確認", MessageBoxButtons.OK, MessageBoxIcon.Information)
